...
from .account_result import AccountResult, Account
...
class AuthHttp(CodefBaseHttp):
    ...
    async def get_account_list(self, connected_id: Annotated[str, BodyJson.to_camel()]) -> dict:
        pass

    @pydantic_response_model()
    @request("POST", PATH_GET_CID_LIST)
    async def get_cid_list(self, page_no: Annotated[int, BodyJson.to_camel()]) -> dict:
        pass
